...
def _track_step_v2(loop_seg: np.ndarray, release_ds: np.ndarray,
    ...
    # Sort by cumulative score descending; exact float64 ties broken by beam_id.
    # EPS-based grouping avoided: GO and Python accumulate slightly different
    # float32 values (scalar vs. vectorised NDP), so any EPS > 0 creates a
    # threshold that the two implementations straddle differently.
    new_cands.sort(key=lambda x: (-x[4], x[5]))
    filtered = []
...
def compute_lut_v2(attack_mono: np.ndarray, release_mono: np.ndarray,
    ...
    # Capture final beam states for diagnostic comparison in verify_lut.
    # Format: list of (r_samples, cum_score, beam_id)
    meta["final_beams"] = [(int(b[0] * ds), float(b[4]), int(b[5]))
                           for b in vis_candidates]
