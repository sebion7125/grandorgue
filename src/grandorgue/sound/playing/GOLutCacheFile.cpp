...
bool GOLutCacheWriter::Write(
  ...
  if (releaseMap.size() != releaseCount)
    return false;
  ...
  if (utf8.length() >= GOLUT_HASH_FIELD)
    return false;
  strncpy(hashField, utf8.data(), GOLUT_HASH_FIELD - 1);

  // Pre-calculate buffer size to avoid reallocations.
  size_t bufSize = sizeof(GOLUT_MAGIC) + 4 + 4 + GOLUT_HASH_FIELD + 4
    + 4 // versions + hash + counts
  ...
  for (const auto &e : luts)
    bufSize += sizeof(uint32_t)
      + sizeof(double)   // period_samples + period_float
...
bool GOLutCacheReader::Load(
  ...
  if (!r.Read(magic, sizeof(magic)))
    return false;
  if (memcmp(magic, GOLUT_MAGIC, sizeof(GOLUT_MAGIC)) != 0)
    return false;
  ...
  if (!r.ReadU32(algVer))
    return false;
  if (fmtVer != GOLUT_FORMAT_VERSION)
    return false;
  if (algVer != GOLUT_ALGORITHM_VERSION)
    return false;

  char hashField[GOLUT_HASH_FIELD] = {};
  if (!r.Read(hashField, GOLUT_HASH_FIELD))
    return false;
  hashField[GOLUT_HASH_FIELD - 1] = '\0';
  if (wxString::FromUTF8(hashField) != expectedOdfHash)
    return false;

  uint32_t releaseCount = 0, lutCount = 0;
  if (!r.ReadU32(releaseCount))
    return false;
  if (!r.ReadU32(lutCount))
    return false;
  if (releaseCount != expectedReleaseCount)
    return false;
  if (lutCount > releaseCount)
    return false;
  ...
  if (!r.Read(&m_criteria.minScore, sizeof(m_criteria.minScore)))
    return false;
  if (!r.Read(&m_criteria.minCoherence, sizeof(m_criteria.minCoherence)))
    return false;
  if (!r.Read(&m_criteria.maxGapFills, sizeof(m_criteria.maxGapFills)))
    return false;
  if (!r.Read(
        &m_criteria.requireStabilization,
        sizeof(m_criteria.requireStabilization)))
    return false;
  if (!r.Read(
        &m_criteria.useDriftFallback, sizeof(m_criteria.useDriftFallback)))
    return false;
  if (!r.Read(m_criteria._pad, sizeof(m_criteria._pad)))
    return false;
  ...
  for (int32_t idx : m_releaseMap)
    if (idx != -1 && (uint32_t)idx >= lutCount)
      return false;
  ...
  for (GOLutEntry &entry : m_luts) {
    if (!r.Read(&entry.period_samples, sizeof(entry.period_samples)))
      return false;
    if (!r.Read(&entry.period_float, sizeof(entry.period_float)))
      return false;
    uint32_t n = 0;
    if (!r.ReadU32(n))
      return false;
    if (n > GOLUT_MAX_POINTS)
      return false;
    entry.points.resize(n);
    if (n > 0) {
      for (GOLutPoint &pt : entry.points) {
        if (!r.Read(&pt.loop_pos, sizeof(pt.loop_pos)))
          return false;
        if (!r.Read(&pt.best_r, sizeof(pt.best_r)))
          return false;
        if (!r.Read(&pt.flags, sizeof(pt.flags)))
          return false;
        if (!r.Read(&pt._pad, sizeof(pt._pad)))
          return false;
      }
    }
  }
