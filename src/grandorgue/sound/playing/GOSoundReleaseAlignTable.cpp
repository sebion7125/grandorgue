...
bool GOSoundReleaseAlignTable::Load(GOCache &cache) {
  ...
  // Format v3: n_luts (uint32), period+crossfade, per-LUT: count +
  // {loop_pos,best_r}. Format v4: n_luts | 0x80000000 → same layout but each
  // point also has {flags,_pad}.
  //            best_r stored folded [0,T).
  // Format v5: n_luts | 0x80000000 | 0x40000000 → v4 layout, best_r unfolded
  // [0,2T).
  //            Old v4 caches are loaded in legacy mode (flags cleared →
  //            T-modulus interp).
  ...
  const bool has_unfolded
    = (n_luts_raw & 0x40000000u) != 0; // v5: best_r in [0,2T)
    ...
    for (AttackLut &lut : m_CorrLuts) {
      ...
      for (CorrPoint &cp : lut.points) {
        ...
        if (!cache.Read(&cp.best_r, sizeof(cp.best_r)))
          return false;
        if (has_flags) {
          if (!cache.Read(&cp.flags, sizeof(cp.flags)))
            return false;
          if (!cache.Read(&cp._pad, sizeof(cp._pad)))
            return false;
          // v4 cache has folded best_r [0,T); clear flags so
          // GetPositionForCorrelation uses legacy T-modulus path and the old
...
bool GOSoundReleaseAlignTable::Save(GOCacheWriter &cache) {
  ...
  // ── Sparse correlation LUTs (v5 format: 0x80000000=has_flags,
  // 0x40000000=unfolded) ──
  const uint32_t n_luts_raw = (uint32_t)m_CorrLuts.size() | 0x80000000u
    | 0x40000000u; // v5: flags + unfolded
    ...
    for (const AttackLut &lut : m_CorrLuts) {
      ...
      for (const CorrPoint &cp : lut.points) {
        ...
        if (!cache.Write(&cp.best_r, sizeof(cp.best_r)))
          return false;
        if (!cache.Write(&cp.flags, sizeof(cp.flags)))
          return false;
        if (!cache.Write(&cp._pad, sizeof(cp._pad)))
          return false;
      }
    }
  }
...
// C++ std::round rounds half away from zero, which diverges from Python's
// reference at x == N.5 and causes exp_pos to differ by 1 downsampled unit (=
// ds original samples).
...
// Silent release window (denom < 1e-12) → -2.f, matching Python's np.where(ok,
// ..., -2.0). Used in FullScanV2 / TrackStepV2 to align with Python's float32
// computation.
//
// In verify/logging builds (GOLogReleaseAlignEnable.h present):
// auto-vectorization and FMA contraction are disabled so the float32 reduction
// is byte-identical to Numba's sequential scalar loop (the Python reference).
// Production builds use full -O3.
#if __has_include("GOLogReleaseAlignEnable.h")
#if defined(_MSC_VER)
...
static LOG_NOINLINE float NDP_f32(
  const float *lw_n, const float *rel, unsigned r, unsigned W) {
#if __has_include("GOLogReleaseAlignEnable.h")
...
#endif
#endif
static LOG_NOINLINE bool CalcLwN_f32(const float *lw, unsigned W, float *lw_n) {
#if __has_include("GOLogReleaseAlignEnable.h")
...
// Matches Python's refine_peak_parabolic: delta =
// 0.5*(y[i-1]-y[i+1])/(y[i-1]-2y[i]+y[i+1]).
  ...
  double expected_period) // HN-corrected nominal period (T_hn)
{
  ...
  // NDP for all lags 1..max_period.  ndp_all[lag] == NDP at that lag; index 0
  // unused.
  ...
  // Collect local-minimum valleys in (min_period, max_period) — exclusive
  // endpoints.
  struct Valley {
    float cmndf;
    unsigned lag;
  ...
  if (valleys.empty()) {
    ...
    if (in_v)
      valleys.push_back({best_c, v_lag, ndp_all[v_lag]});
  }
  ...
  if (!valleys.empty()) {
    ...
    for (size_t k = 1; k < valleys.size(); k++) {
      ...
      const bool much_deeper
        = v.cmndf <= std::max(0.050f, chosen.cmndf * 0.35f);
    ...
    for (unsigned lag = min_period; lag <= max_period; lag++) {
      const float sc
        = ndp_all[lag] * (1.f - 0.10f * (float)lag / (float)max_period);
      if (sc > best_score) {
        best_score = sc;
        best_lag = lag;
      }
  ...
  // Submultiple guard: if result ≈ expected_period/k (k=2..8) and
  // expected_period shows good periodicity, override to expected_period (port
  // of Python v75/v76).
  if (expected_period > 0.0) {
    const int exp_i = (int)std::round(expected_period);
    const int res_i = (int)std::round(result);
    if (
      exp_i >= 1 && exp_i <= (int)max_period && res_i >= 1
      && res_i <= (int)max_period) {
      ...
      if (k >= 2 && k <= 8 && std::abs(ratio - (double)k) <= 0.18) {
        ...
        const bool exp_not_worse
          = cm_exp <= cm_res * 1.25f || ndp_exp >= ndp_res - 0.03f;
        const bool exp_clearly_better
          = cm_exp < cm_res * 0.50f || ndp_exp > ndp_res + 0.03f;

        if (
          exp_good && res_suspicious
          && (exp_not_worse || exp_clearly_better || (cm_exp <= 0.05f && ndp_exp >= 0.90f)))
          result
            = 1.0 + RefineNDPPeak(ndp_base, (unsigned)exp_i - 1, max_period);
      }
    }
  }
...
static std::vector<BeamState> FullScanV2(
  ...
  // e_lw dot-product is scalar float32, matching Python
  // _normalize_lw_f32_numba.
  std::vector<float> lw_n(window_d);
  if (!CalcLwN_f32(lw, window_d, lw_n.data()))
    return {};

  // Score for each candidate position (float32, matches Python
  // _compute_corr_scores).
  const unsigned n
    = std::min(r_max_d, rel_d_len > window_d ? rel_d_len - window_d : 0u);
  std::vector<float> sc(n, -2.f);
  for (unsigned r = 0; r < n; r++)
    sc[r] = NDP_f32(lw_n.data(), rel_d, r, window_d);

  // Local maxima with non-max suppression (min distance = 3 downsampled
  // samples, matching Python BRANCH_MIN_PEAK_DISTANCE=3).
  ...
  std::stable_sort(peaks.begin(), peaks.end(), [&](unsigned a, unsigned b) {
    return sc[a] > sc[b];
  });
...
static TrackResult TrackStepV2(
  ...
  for (const BeamState &b : cands) {
    float slope
      = (b.dn_prev > 0) ? (float)(b.r_d - b.r_d_prev) / (float)b.dn_prev : 0.f;
...
static std::vector<V2TrackPt> PruneV2(
  ...
  while (changed) {
    ...
    for (unsigned k = 1; k + 1 < (unsigned)idx.size(); k++) {
      ...
      if (pts[next].n - pts[prev].n > V2_MAX_PRUNE_GAP_N)
        continue;
      // Skip if track_r-discontinuity > T/4 at either side.
      if (
        std::abs(pts[cur].track_r - pts[prev].track_r) > (int)(T_int_f / 4.0f))
        continue;
      if (
        std::abs(pts[next].track_r - pts[cur].track_r) > (int)(T_int_f / 4.0f))
        continue;

      // Douglas-Peucker: ALL original points in (prev, next) must fit within
      // tol. Use unwrapped track_r for endpoints and intermediate values so the
      ...
      for (unsigned j = prev + 1; j < next; j++) {
        ...
        if (std::abs((float)pts[j].track_r - er) > tol) {
          can_del = false;
          break;
        }
      }
...
static unsigned InterpolateCorrPointRaw(
  ...
  if (loop_pos <= (int)pts.front().loop_pos)
    return (unsigned)(pts.front().best_r % W);
  if (loop_pos >= (int)pts.back().loop_pos)
    return (unsigned)(pts.back().best_r % W);
  ...
  // Zirkuläre Interpolation via approach_up — entspricht Python
  // interp_lut_segment_raw. Ohne dieses Bit würde 1380→20 (W=1400) als
  // Rückwärts- statt Vorwärts-Wrap interpoliert (r0 + t*(r1-r0) fällt von 1380
  // auf 20 statt über 0 zu gehen).
...
static unsigned GetPositionForCorrImpl(
  ...
  const unsigned phi
    = (unsigned)std::round(std::fmod((double)loop_pos, T_f)) % T_int;
...
void GOSoundReleaseAlignTable::ComputeCorrelationLut(
  ...
  if (exhaustive)
    permissive = true; // exhaustive implies permissive
#if __has_include("GOLogReleaseAlignEnable.h")
  ...
  double diag_initial_T
    = 0.0; // smpl_T before HN; verify derives hn_T = initial_T * hn/8
  unsigned diag_ac_loop_start = 0; // GetLoopStart() = loops[0][0]
  unsigned diag_loop_end_used = 0; // GetFirstLoopEnd() = loops[0][1]
  unsigned diag_ac_offset
    = 0; // autocorr window start = loop_start + loop_len/2
...
#if __has_include("GOLogReleaseAlignEnable.h")
    diag_initial_T
      = smpl_T_float; // smpl_T (pre-HN); verify derives hn_T = initial_T * hn/8
#endif
  }
  ...
  //   min_p = max(16, round(0.5 * T_hn_int))       ← round(), not integer
  //   divide max_p = min(sr//20, round(2.0 * T_hn_int))   ← round(), not
  //   integer multiply loop_len = GetFirstLoopEnd() - GetLoopStart() + 1  ←
  //   first SMPL loop only ac_offset = loop_start + loop_len // 2 ac_window =
  //   min(8*max_p, buf_end - ac_offset) ← clip at EOF only, no backshift
  if (first_call) {
    ...
    const unsigned min_p
      = std::max(16u, (unsigned)std::round(0.5 * (double)T_hn_int));
    const unsigned max_p = std::min(
      sample_rate / 20u, (unsigned)std::round(2.0 * (double)T_hn_int));
    ...
    if (max_p >= min_p * 2 && T_hn_int >= 16u) {
      ...
      // Python: loop_len = loops[0][1] - loops[0][0] + 1 (first SMPL loop
      // only). Falls back to full-buffer span when GetFirstLoopEnd() returns 0
      // (no loops).
      ...
      // Python: atk[loop_mid:loop_mid+window] — clips at EOF silently, no
      // backshift.
  ...
  // Ensures n_last = n_total-1 stays within the loop buffer (cs + window <=
  // loop_len).
  ...
  const unsigned max_samp_limit
    = (max_key_press_ms > 0) ? max_key_press_ms * sample_rate / 1000 : 0u;
  unsigned n_start = (min_samp > 0)
    ? std::max(1u, (unsigned)std::ceil((double)min_samp / T_f))
    : 1u;
  ...
  const unsigned loop_needed
    = std::min(loop_len, (unsigned)std::round((n_end - 1) * T_f) + window_len);
  const unsigned loop_needed_d = (loop_needed + ds - 1)
    / ds; // ceil(loop_needed/ds) = len(attack[:loop_needed:ds])
  ...
  auto corr_at = [&](unsigned n) -> ScoredPoint {
    ...
    const int hi_d
      = std::min((int)r_max_d, (int)folded_idx_d + FOLD_SEARCH_RADIUS_D + 1);
    ...
    if (raw_score > 1e-12f && folded_score / raw_score >= FOLD_ACCEPT_RATIO)
      return {
        cs, (uint16_t)(folded_r_d * ds % m_CorrPeriodSamples), folded_score};
    return {cs, (uint16_t)(raw_r_d * ds % m_CorrPeriodSamples), raw_score};
  };
  ...
  //            dn = V2_DN_DENSE (n ≤ V2_DENSE_N_LIMIT) or V2_DN_SPARSE
  //            otherwise. Each step: ±1 probe; extends to ±2 when best hits the
  //            edge. Rescan trigger: score drops < 90 % of prev or drift > 8
  //            samples.
  ...
  // Phase 2  : V2TrackPt list → is_jump / approach_up → Douglas-Peucker prune.
  {
    ...
    while (n_cur >= n_start) {
      ...
      const unsigned n_next = (n_cur >= n_start + dn) ? n_cur - dn : n_start;
      if (n_next >= n_cur)
      ...
      if (!res.beams.empty()) {
        ...
        if (res.needs_rescan) {
          ...
          if (!fresh.empty()) {
            ...
            // have locally collapsed — assigning the second peak to the next
            // free beam would create artificial beam crossings and chaotic
            // histories. Instead, discard the second peak (matches Python
            // semantics exactly).
            ...
            for (const BeamState &fr : fresh) {
              ...
              for (const BeamState &vb : vis_cands) {
                if (vb.beam_id < 0 || (unsigned)vb.beam_id >= V2_TOP_K)
                  continue;
              ...
              if (id_used[(unsigned)best_bid])
                continue;
              id_used[(unsigned)best_bid] = true;
              float prev_cum = 0.f;
              for (const BeamState &vb : vis_cands)
                if (vb.beam_id == best_bid) {
                  prev_cum = vb.cum_score;
                  break;
                }
              refreshed.push_back(
    ...
    final_beam_states
      = vis_cands; // capture for CSV diagnostics (beam state after Phase 1)
    ...
    {
      const unsigned sp_T_raw
        = sp_T_d * ds; // = 2*(T_int//ds)*ds; Python uses _sp_T_for_iv =
                       // sp_T_d*ds (same), not _sp_T_raw = 2*T_int (different
                       // variable, pre-Phase-1.5 only)
      const float jump_thresh
      ...
      for (const auto &iv : jump_ivs) {
        ...
        for (int nd = (int)iv.second - 1; nd > (int)iv.first; nd--) {
          ...
          float slope_log = (cb.dn_prev > 0)
            ? (float)(cb.r_d - cb.r_d_prev) / (float)cb.dn_prev
            : 0.f;
          int exp_pos_log
            = RoundHalfEven((float)cb.r_d + slope_log) % (int)sp_T_d;
          if (exp_pos_log < 0)
            exp_pos_log += (int)sp_T_d;
    ...
    for (const auto &kv : primary_path) {
      if (kv.second.second < -1.5f)
        continue;
      V2TrackPt p;
      p.n = kv.first;
      p.r = kv.second.first;
      p.track_r = unwrapped_r_by_n.count(kv.first)
        ? unwrapped_r_by_n.at(kv.first)
        : kv.second.first;
    ...
    auto mark_flags = [&](std::vector<V2TrackPt> &pts) {
      ...
      for (unsigned i = 1; i < (unsigned)pts.size(); i++) {
        ...
        if ((unsigned)dist > sp_T_full / 2u)
          dist = (int)sp_T_full - dist;
    ...
    if (!v2_pts.empty()) {
      ...
      for (unsigned i = 1; i < (unsigned)v2_pts.size(); i++)
        v2_pts[i].approach_up
          = (v2_pts[i].track_r - v2_pts[i - 1].track_r) >= 0;
    }
  }
...
lut_commit:
  ...
  if (!v2_pts.empty()) {
    ...
    for (const V2TrackPt &p : v2_pts) {
      ...
      if (p.approach_up)
        flags |= CorrPoint::kFlagApproachUp;
      if (p.is_jump)
        flags |= CorrPoint::kFlagIsJump;
...
#if __has_include("GOLogReleaseAlignEnable.h")
  {
    ...
    f << "corr_lut"
      << " freq=" << sample_freq_hz << " T=" << m_CorrPeriodSamples
      << " points=" << points.size() << " us=" << us
      << " total_ms=" << (total / 1000) << " n=" << n << "\n";
  }
#endif
  ...
  if (label) {
    ...
    if (!s_Rotated.exchange(true)) {
      ...
      if (std::ifstream(path).good()) {
        ...
        const std::string stem
          = (dot.empty()) ? path : path.substr(0, path.rfind('.'));
    ...
    if (vf.is_open()) {
      ...
      // Before fix: 0.f  After fix: -2.f  Used to verify which binary is
      // running.
      static constexpr float NDP_SILENT_SCORE = -2.f;
      vf << "pipe=" << label << " align_version=v231b"
         << " ndp_silent_score=" << NDP_SILENT_SCORE
         << " T_float=" << std::hexfloat << m_CorrPeriodFloat
         << std::defaultfloat << " T_int=" << m_CorrPeriodSamples
         << " crossfade_len=" << crossfade_len << " ds=" << ds
         << " harmonic=" << harmonic_number << " n_start=" << n_start
         << " n_end=" << n_end << " min_ms=" << min_key_press_ms
         << " max_ms=" << max_key_press_ms << " sample_rate=" << sample_rate
         << " loop_len=" << loop_section.GetLength()
         << " release_len=" << release_section.GetLength() << " r_max=" << r_max
         << " latest_loop_end=" << latest_loop_end << " loop_count=" <<
        [&]() {
          unsigned c = 0;
          for (unsigned i = 0; i < loop_section.GetEndSegmentCount(); i++)
            if (loop_section.GetEndSegment(i).next_start_segment_index >= 0)
              c++;
          return c;
        }()
         << " attack_file=" << loop_section.GetLoaderBasename()
         << " initial_T=" << std::hexfloat << diag_initial_T
         << std::defaultfloat << " ac_loop_start=" << diag_ac_loop_start
         << " loop_end_used=" << diag_loop_end_used
         << " ac_offset=" << diag_ac_offset << " ac_window=" << diag_ac_window
         << " ac_min_p=" << diag_ac_min_p << " ac_max_p=" << diag_ac_max_p
         << "\n";
      ...
      for (const BeamState &b : phase0_cands)
        vf << "phase0," << b.beam_id << "," << (b.r_d * (int)ds) << ","
      ...
      // cum_score_hex: std::hexfloat for exact float32 round-trip (no decimal
      // precision loss).
      for (const BeamState &b : final_beam_states)
        vf << "final_beam," << b.beam_id << "," << (b.r_d * (int)ds) << ","
      ...
      for (const V2TrackPt &p : pre_prune_v2pts)
        vf << "pre_prune," << p.n << "," << p.r << "," << std::setprecision(6)
           << p.score << "," << (p.is_jump ? 1 : 0) << ","
           << (p.approach_up ? 1 : 0) << "\n";
      ...
      for (const Phase15Iv &iv : phase15_ivs)
        vf << "phase15_iv," << iv.na << "," << iv.nb << "," << iv.ra << ","
           << iv.rb << "," << iv.raw_dist << "," << iv.circ_dist << ","
      ...
      for (const Phase15Step &st : phase15_steps)
        vf << "phase15_step," << st.nd << "," << st.cs_d << "," << st.exp_pos
           << "," << st.best_r << "," << std::setprecision(6) << st.best_sc
           << "," << (st.inserted ? 1 : 0) << "\n";
      // Post-prune points (after PruneV2 + recomputed mark_flags).
      // Format: post_prune,n,r,is_jump,approach_up
      for (const V2TrackPt &p : v2_pts)
        vf << "post_prune," << p.n << "," << p.r << "," << (p.is_jump ? 1 : 0)
           << "," << (p.approach_up ? 1 : 0) << "\n";
      for (const CorrPoint &p : pts) {
        const unsigned n = (unsigned)std::round((double)p.loop_pos / T_f);
        // best_r is unfolded [0,2T) for v2 path; legacy exhaustive path keeps
        // [0,T).
        vf << "lut," << n << "," << p.loop_pos << "," << p.best_r << ","
           << (p.IsApproachUp() ? 1 : 0) << "," << (p.IsJump() ? 1 : 0) << "\n";
      }
      ...
      for (unsigned n = n_start; n < n_end; n += sim_step) {
        ...
        vf << "sim," << lp << ","
           << GetPositionForCorrImpl(
                lp, pts, m_CorrPeriodFloat, m_CorrPeriodSamples)
           << "\n";
        last_lp = lp;
      }
      ...
      if (end_lp != last_lp)
        vf << "sim," << end_lp << ","
           << GetPositionForCorrImpl(
                end_lp, pts, m_CorrPeriodFloat, m_CorrPeriodSamples)
           << "\n";
    }
  }
#endif
...
const std::vector<GOSoundReleaseAlignTable::CorrPoint>
  *GOSoundReleaseAlignTable::FindLut(
...
unsigned GOSoundReleaseAlignTable::GetPositionForCorrelation(
  ...
  const double T_f = (m_CorrPeriodFloat > 0.0) ? m_CorrPeriodFloat
                                               : (double)m_CorrPeriodSamples;
  // Guard: round() can produce exactly T_int when fmod result is just below
  // T_f.
...
void GOSoundReleaseAlignTable::OverrideCorrLutsFromCache(
  std::vector<CorrPoint> points, uint32_t period_samples, double period_float) {
  if (points.empty())
    return;
