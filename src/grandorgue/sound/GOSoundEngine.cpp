      ...
      if (not_a_tremulant) {
        ...
        //if (m_ScaledReleases) {
          ...
          if (chamade) {
            switch (chan) {
              case CK_Dry:
                attack_duration  = tmax_dry_by_midi[midikey_frequency];   /* spezielle Release-Parameter für Chamade Dry */
                g_0 = g0_dry_by_midi[midikey_frequency];
                CHAMADE_DEBUG("Release gestartet: Cham Dry ");
                break;
              case CK_Front:
                attack_duration  = tmax_front_by_midi[midikey_frequency];   /* spezielle Release-Parameter für Chamade Dry */ 
                g_0 = g0_front_by_midi[midikey_frequency];
                CHAMADE_DEBUG("Release gestartet: Cham Front ");
                break;
              case CK_Rear:
                attack_duration  = tmax_rear_by_midi[midikey_frequency];   /* spezielle Release-Parameter für Chamade Dry */
